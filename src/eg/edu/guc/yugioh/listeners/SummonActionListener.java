...
public class SummonActionListener implements ActionListener{
	...
	public SummonActionListener(GUI gui) {
				...
				if(B.getActivePlayer().getName().equals(gui.getP1().getPlayerStatusPanel().getNameOfPlayer().getText())){
					try{
						...
						if(c instanceof MonsterCard){
							...
							if(level >= 1 && level <= 4){
								B.getActivePlayer().summonMonster((MonsterCard) B.getActivePlayer().getField().getHand().get(handMonsterButtonIndex));
							...
							if(!B.getActivePlayer().getField().getMonstersArea().isEmpty()){
								if(level >= 5 && level <= 6){
									if(e.getSource() instanceof MonsterCardButton){
										...
										B.getActivePlayer().summonMonster((MonsterCard) B.getActivePlayer().getField().getHand().get(handMonsterButtonIndex), sacrifices);
									}
									for (int i = 0; i < B.getActivePlayer().getField().getMonstersArea().size(); i++) {
